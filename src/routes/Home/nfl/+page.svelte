<script>
	import { getData } from '$lib/util/util';
	import { doc, onSnapshot, getFirestore } from 'firebase/firestore';
	import { app, auth, getDocument } from '$lib/fire_base/firebase';
	import { allSiteNflData, lastPage } from '$lib/stores/ketzner';
	...
	let selected = "/Home/nfl";
	const db = getFirestore(app);
	let res;
	let waitModal;
	const unsub = onSnapshot(doc(db, 'NFLUpdateTime', 'time'), (doc) => {
		res = getData();
	});
...
{#if res != null}
	{#await res}
		{#if Object.keys($allSiteNflData).length === 0}
					...
					<table class="table p-5">
						<thead class="table-dark sticky-top">
							<tr>
								...
								{#each $allSiteNflData.sitesList as site}
									<td
										><a
						...
						<tbody>
							{#each $allSiteNflData.games as game, i}
								<tr>
									<td>{i + 1}</td>
